...
export default function Blog() {
  ...
  return (
    <div className="flex">
      <div className="w-[70%] h-screen p-5">
          <h1 className="text-center text-4xl font-bold mb-5">{blog?.title}</h1>
          <div className="font-semibold">{blog?.content}</div>
      </div>
      <div className="w-[30%] bg-gray-200 h-screen flex justify-center items-center p-4 gap-4">
        <div>
          ...
          <div className="flex gap-5 items-center">
            ...
            <div>
              <h1 className="text-4xl font-bold">{blog?.author.name}</h1>
              <div>{Quotes[Math.floor(Math.random() * 4)]}</div>
            </div>
          </div>
        </div>
      </div>
